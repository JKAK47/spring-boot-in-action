    ...
    class DataJedisProperties {
        ...
        JedisConnectionFactory jedisConnectionFactory(JedisPoolConfig jedisPoolConfig) {
            ...
            JedisClientConfiguration jedisClientConfiguration = JedisClientConfiguration.builder().
                    clientName("JedisClient").readTimeout(Duration.ofMillis(timeout)).connectTimeout(Duration.ofMillis(timeout)).usePooling()
                    .poolConfig(jedisPoolConfig).and().build();

            /**用于配置创建Redis 链接 的设置；
             * 设置主机ip，端口，密码，以及使用Redis db 序号 */
            RedisStandaloneConfiguration connConfiguration = new RedisStandaloneConfiguration();
            connConfiguration.setHostName(host);
            connConfiguration.setPort(port);
            connConfiguration.setPassword(RedisPassword.of(password));
            /** 设置使用redis 索引 */
            connConfiguration.setDatabase(2);
            /** Jedis ConnectionFactory */
            JedisConnectionFactory factory = new JedisConnectionFactory(connConfiguration, jedisClientConfiguration);
            return factory;
        }
